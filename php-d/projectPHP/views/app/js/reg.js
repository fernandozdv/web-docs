function goReg(){
	...
	if(true== tyc)
	{
		if(user!=''&&pass!=''&&pass_dos!=''&&email!='')
		{
				...
				form = 'user='+user+'&pass='+pass+'&email='+email;
					...
					connect.onreadystatechange = function()
					{
						...
						else if(connect.readyState !=4)
						{
				            ...
				            result+='<p><strong>Estamos intentado registrarte!</strong></p></div>';
							__('_AJAX_REG_').innerHTML=result;
						}
					}
	        ...
	        result+='<button type="button" class="close" data-dismiss="alert">&times;</button>';
	        result+='<h4>ERROR.</h4>';
	        result+='<p><strong>Todos los campos deben estar llenos.</strong></p></div>';
	        __('_AJAX_REG_').innerHTML=result;
		}
	}
	else
	{
        ...
        result+='<button type="button" class="close" data-dismiss="alert">&times;</button>';
        result+='<h4>ERROR.</h4>';
        result+='<p><strong>Los términos y condiciones deben ser aceptados.</strong></p></div>';
        __('_AJAX_REG_').innerHTML=result;
	}
}	
